...
def generate_component_in_salary_slip_update(doc, method):
	...
	if not doc.name.upper().startswith("NEW"):
		base = None
		list = []
...
def calculate_amount(base,no_of_hours,salary_component):
	...
	#frappe.msgprint(salary_component)
	if component and component.based_on_hourly_rate:
		calc = (float(base)/ float(working_hours_per_month) ) *  float(no_of_hours) * (float(component.based_on_hourly_rate) / 100)
		return calc
	else:
